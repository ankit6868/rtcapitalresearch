...
async function sb(): Promise<SupabaseClient> {
  // Auto-run schema migration on first call. No-op after first success.
...
export const supabaseBackend = {
  ...
  async getSettings(): Promise<Settings> {
    const { data, error } = await (await sb()).from("settings").select("data").eq("id", 1).maybeSingle();
    if (error) throw error;
    if (!data) {
      await (await sb()).from("settings").insert({ id: 1, data: DEFAULT_SETTINGS });
      return DEFAULT_SETTINGS;
    }
    return data.data as Settings;
  },
  async saveSettings(s: Settings) {
    const { error } = await (await sb()).from("settings").upsert({ id: 1, data: s, updated_at: new Date().toISOString() });
    if (error) throw error;
  },

  // ---------- Sections ----------
  async getSections(): Promise<Section[]> {
    const { data, error } = await (await sb()).from("sections").select("*").order("order", { ascending: true });
    if (error) throw error;
    if (!data || data.length === 0) {
      // seed defaults
      await (await sb()).from("sections").insert(DEFAULT_SECTIONS);
  ...
  async saveSections(arr: Section[]) {
    ...
    const { error } = await (await sb()).from("sections").upsert(arr);
    if (error) throw error;
  },
  async updateSection(id: string, patch: Partial<Section>): Promise<Section | null> {
    const { data, error } = await (await sb()).from("sections").update(patch).eq("id", id).select().maybeSingle();
    if (error) throw error;
    return (data as Section) || null;
  },
  async deleteSection(id: string) {
    const { error } = await (await sb()).from("sections").delete().eq("id", id);
    if (error) throw error;
  },
  async addSection(input: { title?: string; key?: string; content?: Record<string, unknown> }) {
    ...
    const { error } = await (await sb()).from("sections").insert(row);
    if (error) throw error;
  },

  // ---------- Nav (single row id=1, jsonb array of items) ----------
  async getNav(): Promise<NavItem[]> {
    const { data, error } = await (await sb()).from("nav").select("items").eq("id", 1).maybeSingle();
    if (error) throw error;
    if (!data) {
      await (await sb()).from("nav").insert({ id: 1, items: DEFAULT_NAV });
      return DEFAULT_NAV;
    }
    return data.items as NavItem[];
  },
  async saveNav(arr: NavItem[]) {
    const { error } = await (await sb()).from("nav").upsert({ id: 1, items: arr, updated_at: new Date().toISOString() });
    if (error) throw error;
  },

  // ---------- Footer (single row id=1, jsonb array of columns) ----------
  async getFooter(): Promise<FooterColumn[]> {
    const { data, error } = await (await sb()).from("footer").select("columns").eq("id", 1).maybeSingle();
    if (error) throw error;
    if (!data) {
      await (await sb()).from("footer").insert({ id: 1, columns: DEFAULT_FOOTER });
      return DEFAULT_FOOTER;
    }
    return data.columns as FooterColumn[];
  },
  async saveFooter(arr: FooterColumn[]) {
    const { error } = await (await sb()).from("footer").upsert({ id: 1, columns: arr, updated_at: new Date().toISOString() });
    if (error) throw error;
  },

  // ---------- Enquiries ----------
  async getEnquiries(): Promise<Enquiry[]> {
    const { data, error } = await (await sb()).from("enquiries").select("*").order("created_at", { ascending: false });
    if (error) throw error;
    return (data as EnquiryRow[]).map(rowToEnquiry);
  },
  async addEnquiry(e: Omit<Enquiry, "id" | "createdAt" | "status">): Promise<Enquiry> {
    const { data, error } = await (await sb()).from("enquiries").insert({
  ...
  async updateEnquiry(id: string, patch: Partial<Enquiry>): Promise<Enquiry | null> {
    ...
    const { data, error } = await (await sb()).from("enquiries").update(dbPatch).eq("id", id).select().maybeSingle();
    if (error) throw error;
    return data ? rowToEnquiry(data as EnquiryRow) : null;
  },
  async deleteEnquiry(id: string) {
    const { error } = await (await sb()).from("enquiries").delete().eq("id", id);
    if (error) throw error;
  },

  // ---------- Admin ----------
  async getAdmin(): Promise<Admin> {
    const { data, error } = await (await sb()).from("admin").select("username, password_hash").eq("id", 1).maybeSingle();
    if (error) throw error;
    if (!data) {
      const seed = { username: "admin", passwordHash: sha256("admin123") };
      await (await sb()).from("admin").insert({ id: 1, username: seed.username, password_hash: seed.passwordHash });
      return seed;
    }
  ...
  async updatePassword(newPassword: string) {
    const { error } = await (await sb()).from("admin").upsert({ id: 1, username: "admin", password_hash: sha256(newPassword), updated_at: new Date().toISOString() });
    if (error) throw error;
  },
};
